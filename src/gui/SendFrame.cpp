...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("prosus://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "prosus:");
    }
    if(!_request.startsWith("prosus:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with 'prosus' :"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "Prosus38FZ6VGSptmp5RBrgiMYvaRexx9T5Bz8D3PKwGQ1aGue55mSW8iheFvLAPuDDWWBnDJZN2Q1a8PZGKbWhCAogPfYxoYLE4f";
