...
namespace WalletGui {
    ...
    NodeAdapter::NodeAdapter()
            : QObject(), m_node(nullptr), m_nodeInitializerThread(), m_nodeInitializer(new InProcessNodeInitializer),
              m_daemonHost(std::string()) {
        ...
        connect(m_nodeInitializer, &InProcessNodeInitializer::nodeInitCompletedSignal, this,
                &NodeAdapter::nodeInitCompletedSignal, Qt::QueuedConnection);
        connect(this, &NodeAdapter::initNodeSignal, m_nodeInitializer, &InProcessNodeInitializer::start,
                Qt::QueuedConnection);
        connect(this, &NodeAdapter::deinitNodeSignal, m_nodeInitializer, &InProcessNodeInitializer::stop,
                Qt::QueuedConnection);
    }
    ...
    bool NodeAdapter::init() {
        ...
        if (connection.compare("embedded") == 0) {
            ...
            QUrl localNodeUrl = QUrl::fromUserInput(
                    QString("127.0.0.1:%1").arg(Settings::instance().getCurrentLocalDaemonPort()));
            m_daemonHost.assign(localNodeUrl.host().toStdString());
            m_daemonPort = localNodeUrl.port();
            m_node = createRpcNode(CurrencyAdapter::instance().getCurrency(), *this,
                                   LoggerAdapter::instance().getLoggerManager(), m_daemonHost,
                                   localNodeUrl.port());
            QTimer initTimer;
            ...
            m_daemonHost.assign(remoteNodeUrl.host().toStdString());
            m_daemonPort = remoteNodeUrl.port();
            m_node = createRpcNode(CurrencyAdapter::instance().getCurrency(), *this,
                                   LoggerAdapter::instance().getLoggerManager(), m_daemonHost,
                                   remoteNodeUrl.port());
            QTimer initTimer;
    ...
    bool NodeAdapter::initInProcessNode() {
        ...
        Q_EMIT initNodeSignal(&m_node, &CurrencyAdapter::instance().getCurrency(), this,
                              &LoggerAdapter::instance().getLoggerManager(), coreConfig, netNodeConfig);
        QEventLoop waitLoop;
    ...
    CryptoNote::NetNodeConfig NodeAdapter::makeNetNodeConfig() const {
        ...
        options.insert(
                std::make_pair("p2p-external-port", boost::program_options::variable_value(p2pExternalPort, false)));
        options.insert(
                std::make_pair("allow-local-ip", boost::program_options::variable_value(p2pAllowLocalIp, false)));
        ...
        if (!priorityNodeList.empty()) {
            options.insert(std::make_pair("add-priority-node",
                                          boost::program_options::variable_value(priorityNodeList, false)));
        }

        std::vector<std::string> exclusiveNodeList = convertStringListToVector(
                Settings::instance().getExclusiveNodes());
        if (!exclusiveNodeList.empty()) {
            options.insert(std::make_pair("add-exclusive-node",
                                          boost::program_options::variable_value(exclusiveNodeList, false)));
        }
    ...
    std::string NodeAdapter::getDaemonHost() {
        return m_daemonHost;
    }

    int NodeAdapter::getDaemonPort() {
        return m_daemonPort;
    }
